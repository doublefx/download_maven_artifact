...
# -------------------------------------------------------
#
# Copied and modified by frederic.thomas@gamesys.co.uk
# In order to make it work for artifactory too.
# TODO: Implement ADL API
#
# -------------------------------------------------------
...
DOCUMENTATION = '''
---
module: download_maven_artifact
...
description:
    ...
    - available.
author: "Frederic Thomas (frederic.thomas@gamesys.co.uk)"
requirements:
    - "python >= 2.6"
    - lxml
...
class MavenDownloader:
    ...
    def _find_latest_version_available(self, artifact):
        ...
        xml = self._request(self.base + path, "Failed to download maven-metadata.xml",
                            lambda r: etree.fromstring(r.content))
    ...
    def find_uri_for_artifact(self, artifact):
        ...
        if artifact.is_snapshot():
            ...
            for snapshotArtifact in xml.xpath("/metadata/versioning/snapshotVersions/snapshotVersion"):
                if len(snapshotArtifact.xpath("classifier/text()")) > 0 and snapshotArtifact.xpath("classifier/text()")[
                    0] == artifact.classifier and len(snapshotArtifact.xpath("extension/text()")) > 0 and \
    ...
    def _uri_for_artifact(self, artifact, version=None):
        ...
        if artifact.classifier:
            return posixpath.join(self.base, artifact.path(),
                                  artifact.artifact_id + "-" + version + "-" + artifact.classifier + "." + artifact.extension)

        return posixpath.join(self.base, artifact.path(),
                              artifact.artifact_id + "-" + version + "." + artifact.extension)
    ...
    def verify_md5(self, file, remote_md5):
        if not os.path.exists(file):
            ...
            remote = self._request(remote_md5, "Failed to download MD5", lambda r: r.raw.read())
...
def main():
    module = AnsibleModule(
        argument_spec=dict(
            repository_name=dict(default=None),
    ...
    repository_url = module.params["repository_url"] + '/' + module.params["repository_name"]
    ...
    try:
        if downloader.download(artifact, dest):
            module.exit_json(state=state, dest=dest, group_id=group_id, artifact_id=artifact_id, version=version,
                             classifier=classifier, extension=extension, repository_url=repository_url, changed=True)
        else:
